...
class linkedinspiderSpider(scrapy.Spider):
    ...
    def parse_profile(self, response):
        ...
        try:
            ...
            item["location"] = (
                summary_box.css("span.top-card__subline-item::text").get().strip()
            )
        ...
        for block in experience_blocks:
            ...
            experience["organisation_profile"] = (
                block.css("h4 a::attr(href)").get(default="").split("?")[0]
            )

            ## location
            experience["location"] = (
                block.css("p.experience-item__location::text").get(default="").strip()
            )
            ...
            try:
                ...
                try:
                    experience["description"] = (
                        block.css("p.show-more-less-text__text--less::text")
                        .get()
                        .strip()
                    )
                except Exception as e:
            ...
            try:
                ...
                if len(date_ranges) == 2:
                    ...
                    experience["duration"] = block.css(
                        "span.date-range__duration::text"
                    ).get()
        ...
        for block in education_blocks:
            ...
            education["organisation_profile"] = (
                block.css("a::attr(href)").get(default="").split("?")[0]
            )
            ...
            try:
                ...
                for text in block.css("h4 span::text").getall():
                    education["course_details"] = (
                        education["course_details"] + text.strip() + " "
                    )
            ...
            education["description"] = (
                block.css("div.education__item--details p::text")
                .get(default="")
                .strip()
            )
